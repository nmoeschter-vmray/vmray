...
class VMRayConnector(BaseConnector):
    ...
    def _get_iocs(
        self, action_result, sample_id: int, timeout: int, all_artifacts: bool = False
    ) -> Union[
    ...
    def _handle_get_iocs(self, param: Dict[str, Any]) -> bool:
        ...
        status, res = self._get_iocs(action_result, sample_id, timeout, all_artifacts=all_artifacts)
        if phantom.is_fail(status):
            if res:
                error_msg, exc = res
        ...
        for key, value in iocs.items():
            action_result.add_data({key: value})

        action_result.update_summary(iocs)

        return action_result.set_status(phantom.APP_SUCCESS)

    def _get_vtis(
        self, action_result, sample_id: int, timeout: int
    ) -> Union[
        Tuple[bool, Optional[Tuple[str, Optional[Exception]]]],
        Tuple[bool, Dict[str, Any]],
    ]:
        ...
        vtis_: List = []
        try:
            vtis_ = api.get_sample_threat_indicators(sample_id).get(
                "threat_indicators", []
            )
        except Exception as exc:  # pylint: disable=broad-except
        ...
        for indicator in vtis_:
            data = {}
            for key, value in indicator.items():
                if key.lower() in THREAT_INDICATOR_FIELDS:
                    data[key] = value
            if data:
                vtis.append(data)
    ...
    def _handle_get_vtis(self, param: Dict[str, Any]) -> bool:
        ...
        status, res = self._get_vtis(action_result, sample_id, timeout)
        if phantom.is_fail(status):
            if res:
                error_msg, exc = res
                action_result.set_status(phantom.APP_ERROR, error_msg, exc)
        ...
        for vti in vtis:
            action_result.add_data({"vti": vti})
